...
def validateMoveWP(pos1row, pos1column, pos2row, pos2column, board):  # 0 = black, 1 = white
    flagleft = False
    flagright = False
    if board[pos1row][pos1column].piece == "P" and board[pos1row][pos1column].colour == "W":  # checks if its whitepawn
        ...
        if board[pos1row - 1][pos1column - 1].colour == "B":  # checks if blackpiece is in capture  range
            flagleft = True
...
def validateMoveBP(pos1row, pos1column, pos2row, pos2column, board):
    ...
    if board[pos1row][pos1column].colour == "B" and board[pos1row][pos1column].piece == "P":
        if board[pos1row][pos1column].hasMoved == False:
        ...
        if board[pos1row+1][pos1column] == board[pos2row][pos2column] and board[pos2row][pos2column].piece == "empty":
            return True
        if board[pos1row + 1][pos1column - 1].piece == "P" and board[pos1row + 1][pos1column - 1].colour == "W":
            flagleftB = True
        if board[pos1row + 1][pos1column + 1].piece == "P" and board[pos1row + 1][pos1column + 1].colour == "W":
            flagrightB = True
        if board[pos2row][pos2column] == board[pos1row + 1][pos1column] and board[pos2row][pos2column].colour == "B":
            return False
        if board[pos2row][pos2column].piece == "empty":
            if pos2row == pos1row + 1 and pos2column == pos1column:
                return True
